...
#### Brownfield: mine the spec before you change it

When the delivery modifies code that already exists, the behaviour it has today is an unwritten
spec — and the Test Case table above only covers what is being *added*. Anything the change
silently alters is untested by construction. So before specifying new rows, inventory the
current observable behaviour of every function, endpoint, or component the change touches:

| Current Behaviour | Where (file:symbol) | Depended on by | Disposition |
|-------------------|---------------------|----------------|-------------|

- **Current Behaviour**: what a caller observes today — return value, status code, persisted
  state, emitted error. Read it out of the code and its callers; never assume it from the name.
- **Depended on by**: the callers found by reference search. `none found` is a valid entry, but
  only after searching — it is the evidence that dropping the behaviour is safe.
- **Disposition**: `Preserve` · `Change` · `Drop`.

Then fold the inventory into the Test Case Specification:
- Every **Preserve** row becomes a characterization row. It passes the moment it is written and
  goes red only if the change breaks it — that is its whole purpose. It still needs a
  **Falsified By** entry naming the break in the *existing* code, so it is proven to be
  load-bearing rather than vacuous.
- Every **Change** row needs two entries: one asserting the old behaviour is gone, one asserting
  the new behaviour holds. A change with only the second is indistinguishable from a no-op.
- Every **Drop** row must name the caller search that found nothing. A dropped behaviour with an
  unsearched caller list is an outage waiting for the next release.

Skip this table only for greenfield components with no existing callers, and say so explicitly
