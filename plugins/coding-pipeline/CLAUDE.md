...
- **Context ceiling — 80%, hard.** Model reliability degrades before the window is full: mid-context recall drops and confident invention rises, which in a pipeline propagates through every stage downstream. At 80%, checkpoint and continue — no human step: write the `PROGRESS.md` entries, commit and push the branch, carry on; the harness compacts, and `precompact-snapshot.sh` + `session-bootstrap.sh` restore state. Never carry on past it without that checkpoint, and never record session state in the code — no `TODO`, no `FIXME`, no commented-out stub marking where an agent stopped. That state belongs to the Memory leg; a marker left in a source file is a CD6 finding in the next review.
