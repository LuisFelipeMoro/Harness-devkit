...
- **Sensors** (feedback): linters in ERROR mode + test runners that return exit 0/1, never prose to interpret. `git-hooks/pre-commit` (format+lint), `git-hooks/pre-push` (tests+coverage+vuln), and CI mirror each other. A task is not done until the sensors pass. Session-level sensors run as hooks (`hooks/hooks.json`) and are deterministic — no model in the loop:
  ...
  | `dispatch-budget.sh` | PreToolUse(Agent) | `pre:agent:dispatch-budget` | Counts subagent dispatches, warns once past the session budget |
...
- **Context ceiling — 80%, hard, and now measured.** `hooks/context-budget.sh` reads the real
  ...
  itself. Model reliability degrades before the window is full: mid-context recall drops and confident invention rises, which in a pipeline propagates through every stage downstream. At 80%, stop and `/handoff`: write the `PROGRESS.md` entries, push the branch, resume in a fresh session from the delivery file's Status. Never carry on past it, and never record session state in the code — no `TODO`, no `FIXME`, no commented-out stub marking where an agent stopped. That state belongs to the Memory leg; a marker left in a source file is a CD6 finding in the next review.
...
- **Per session: 12 dispatches, advisory** — `hooks/dispatch-budget.sh` counts and warns once. The
