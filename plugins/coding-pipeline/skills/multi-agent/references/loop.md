...
> **Each story gets its own branch** — `feat/{key}-{story-slug}`, cut from the release branch — and its own PR into `release/*` once the Verdict passes. That story PR is the review with the tightest spec and the smallest diff, so it is the one most likely to catch something; `/pr-review` runs there with the story's ACs, Test Case table, Reuse Map and Blast Radius in context. Merge with `--no-ff` so the story stays legible as a unit in the release history. Branch table and commands: `../../references/delivery-and-worktree.md`.
