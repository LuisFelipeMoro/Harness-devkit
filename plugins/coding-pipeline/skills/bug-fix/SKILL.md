...
Machine-checkable behavior contract: `skill.spec.yml` · dependency ledger: `deps.toml` (both generated by the `skillspec` CLI and git-ignored — Claude never loads them). Sub-agent dispatch prompts and report templates: [references/dispatch.md](references/dispatch.md).
