...
#
# Agents (plugins/coding-pipeline/agents/*.md) are Claude Code Task-tool subagent
...
# plugins/coding-pipeline/codex/harness-adapter.md for the current mapping and known
...
#
# Platform support:
#   Linux / macOS  — bash plugins/coding-pipeline/scripts/install-codex.sh
#
# Usage:
#   bash plugins/coding-pipeline/scripts/install-codex.sh              # installs to ~/.agents
#   AGENTS_HOME=/tmp/.agents-test bash .../install-codex.sh     # custom target (testing)
set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PLUGIN="$(cd "$SCRIPT_DIR/.." && pwd)"                 # plugins/coding-pipeline
...
if [ -f "$PLUGIN/codex/harness-adapter.md" ]; then
    cp "$PLUGIN/codex/harness-adapter.md" "$TARGET/harness-adapter.md"
...
echo ""
echo "Notes:"
echo "  - Pipeline skills (multi-agent, task, bug-fix,"
...
echo "    see plugins/coding-pipeline/codex/harness-adapter.md for what's still unvalidated"
