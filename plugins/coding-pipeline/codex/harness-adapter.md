...
|---|---|---|
| `Agent` tool, `subagent_type: coding-pipeline:<name>` | Codex custom subagent (`~/.codex/agents/<name>.toml` or `<repo>/.codex/agents/<name>.toml`) | Codex has a real per-file declarative subagent schema (`name`/`description`/`developer_instructions`/`model`/`model_reasoning_effort`, GA'd 2026-03) — see `scripts/generate-codex-agents.sh`, which generates one `<name>.toml` per pipeline persona from `agents/*.md`, with `developer_instructions` set to that persona's full body. No longer an approximation. |
...
```bash
bash plugins/coding-pipeline/scripts/generate-codex-agents.sh              # -> ~/.codex/agents/*.toml
CODEX_HOME=/tmp/.codex-test bash .../generate-codex-agents.sh      # custom target, for testing
```

Writes one `<name>.toml` per pipeline persona (17 files — `coder.md` is core-only and never
...
Every pipeline persona now has a real Codex subagent definition at the correct reasoning
...
phase-by-phase orchestration (`multi-agent`, `task`,
...
- `multi-agent` (9-agent planning-to-verdict pipeline: Analyst → PM → Architect →
