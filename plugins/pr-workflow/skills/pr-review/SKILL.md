...
**Outputs**: inline review comments posted via `gh pr review`; a printed summary block with severity counts, the diff size, the duplication figure, and an action verdict; a **Gaps block** (unimplemented ACs · spec drift · missing Test Case rows · Reuse Map departures · duplication · untraced changes) printed and posted as one general comment.
